<?php
	...
	{	
		...
		if (isset($uploaddata[0]))	{echo "</br>Τα ακόλουθα αρχεία :</br>";}
		for ($x=0; $x<=3; $x++) {
		 ...
		 if (isset($uploaddata[0]))	{echo "ανέβηκαν με επιτυχία στην αναφορά σας.</br>";}
	}
	...
	function table_status_title ($stat) {
		switch($stat){
		case 'fixed':
			$table_title = 'Επιλυμένες';
			break;
		case 'none':
			$table_title = 'Ανοικτές';
			break;
		case 'deleted':
			$table_title = 'Διεγραμμένες';
			break;
		default:
			$table_title = '';
			break;
